...
CREATE OR REPLACE VIEW MOBILE_VIEW_MAIN AS
    SELECT A.MO_NO NO, A.MO_CODE , A.MO_NAME , A.MO_NAME_EN , C.BRAND_NAME BRAND, B.MO_RELEASE ,
        D.OS_NAME , B.MO_OSVER OSë²„ì ¼, B.MO_MATERIAL ì£¼ìš”?ž¬ì§?, B.MO_SIZE ?¬ê¸?, B.MO_WEIGHT ë¬´ê²Œ,
        B.MO_LINK ?—°ê²°ë‹¨?ž, B.MO_INCH ?•¡? •?¬ê¸?, B.MO_RESOL ?™”ë©´í•´?ƒ?„, B.MO_PIXEL ?”½???¸ì¹?, B.MO_DISTYPE ?™”ë©´í??ž…,
        B.MO_DISWIDTH ?™”ë©´í­, B.MO_DISHEIGHT ?™”ë©´ë†’?´, B.MO_AP AP, B.MO_CPU CPU, B.MO_CPUCORE CPUì½”ì–´,
        B.MO_CPUCLOCK CPU?´?Ÿ­, B.MO_GPU GPU, B.MO_RAM RAM, B.MO_INMEMO ?‚´?ž¥ë©”ëª¨ë¦?, B.MO_OUTMEMO ?™¸?ž¥ë©”ëª¨ë¦?,
        B.MO_SENSOR ì¹´ë©”?¼?„¼?„œ, B.MO_IRIS ì¹´ë©”?¼ì¡°ë¦¬ê°?, B.MO_FLASH ì¹´ë©”?¼?”Œ?ž˜?‰¬, B.MO_PICRESOL ?‚¬ì§„í•´?ƒ?„,
...
UNION
SELECT C.*, '3' AS CTABLENO, USER_NAME FROM COM_COMM C
JOIN USER_TABLE U ON ( CC_WIRTER = U.USER_NO )
UNION
SELECT M.*, '2' AS CTABLENO, USER_NAME FROM MO_COMM M
...
SELECT * FROM ADMIN_COMMENT_VIEW where ctableno = 3;
...
(SELECT m.*, USER_NAME 
FROM COM_COMM m
JOIN USER_TABLE U ON ( CC_WIRTER = U.USER_NO )  ORDER BY CC_REGDATE DESC) A) 
WHERE RNUM BETWEEN 1 AND 10;

SELECT * FROM (SELECT ROWNUM RNUM, A.* FROM (SELECT CC_NO CNO, CC_CONTENT CCONTENT, CC_REGDATE CREGDATE, CC_MODIDATE CMODIDATE, CC_CNO AS "REF_NO", CC_WIRTER CWRITER,CC_STATUS CSTATUS, 3 AS "CTABLENO", USER_NAME FROM COM_COMM m JOIN USER_TABLE U ON ( CC_WIRTER = U.USER_NO )  ORDER BY CREGDATE DESC) A) WHERE RNUM BETWEEN 1 AND 10;

SELECT * FROM (SELECT ROWNUM RNUM, A.* FROM (SELECT T.*  FROM (SELECT CNO, CCONTENT, CREGDATE, CMODIDATE,  BNO REF_NO, CWRITER, CSTATUS, '1' AS CTABLENO, USER_NAME FROM COMMENTT JOIN USER_TABLE U ON ( CWRITER = U.USER_NO ) UNION SELECT C.*, '2', USER_NAME FROM COM_COMM C JOIN USER_TABLE U ON ( CC_WIRTER = U.USER_NO ) UNION SELECT M.*, '3', USER_NAME FROM MO_COMM M JOIN USER_TABLE U ON ( MCO_USERNO = U.USER_NO )) T ORDER BY CREGDATE DESC) A where ctableno = 3 ) WHERE RNUM BETWEEN 1 AND 10;

CREATE OR REPLACE VIEW ADMIN_REVIEW_VIEW
AS
SELECT ROWNUM RNUM, A.*
FROM
(SELECT R.*, MO_NAME, (SELECT COUNT(*) FROM REVIEW_LIKE WHERE RE_NO = LIKE_RNO) LCOUNT, BRAND_NAME, USER_NAME FROM REVIEW R
LEFT JOIN MOBILE ON ( RE_MNO = MO_NO )
LEFT JOIN BRAND ON ( MO_BRANDCODE = BRAND_CODE )
LEFT JOIN USER_TABLE ON ( RE_WRITER = USER_NO )
ORDER BY RE_NO DESC) A;

CREATE OR REPLACE VIEW ADMIN_REVIEW_VIEW_ASC
AS
SELECT ROWNUM RNUM, A.*
FROM
(SELECT R.*, MO_NAME, (SELECT COUNT(*) FROM REVIEW_LIKE WHERE RE_NO = LIKE_RNO) LCOUNT, BRAND_NAME, USER_NAME FROM REVIEW R
...
LEFT JOIN BRAND ON ( MO_BRANDCODE = BRAND_CODE )
LEFT JOIN USER_TABLE ON ( RE_WRITER = USER_NO )
ORDER BY RE_NO ASC) A;

CREATE OR REPLACE VIEW ADMIN_REVIEW_VIEW_LCOUNT_DESC
AS
SELECT ROWNUM RNUM, A.*
FROM
(SELECT R.*, MO_NAME, (SELECT COUNT(*) FROM REVIEW_LIKE WHERE RE_NO = LIKE_RNO) LCOUNT, BRAND_NAME, USER_NAME FROM REVIEW R
...
LEFT JOIN BRAND ON ( MO_BRANDCODE = BRAND_CODE )
LEFT JOIN USER_TABLE ON ( RE_WRITER = USER_NO )
ORDER BY LCOUNT DESC) A;

CREATE OR REPLACE VIEW ADMIN_REVIEW_VIEW_LCOUNT_ASC
AS;
SELECT ROWNUM RNUM, A.*
FROM
(SELECT R.*, MO_NAME, (SELECT COUNT(*) FROM REVIEW_LIKE WHERE RE_NO = LIKE_RNO) LCOUNT, BRAND_NAME, USER_NAME FROM REVIEW R
...
LEFT JOIN BRAND ON ( MO_BRANDCODE = BRAND_CODE )
LEFT JOIN USER_TABLE ON ( RE_WRITER = USER_NO )
ORDER BY LCOUNT ASC) A;

SELECT * FROM ADMIN_REVIEW_VIEW WHERE RNUM BETWEEN 1 AND 10;
SELECT * FROM ADMIN_REVIEW_VIEW_ASC WHERE RNUM BETWEEN 1 AND 10;
SELECT * FROM ADMIN_REVIEW_VIEW_LCOUNT_DESC WHERE RNUM BETWEEN 1 AND 10;
SELECT * FROM ADMIN_REVIEW_VIEW_LCOUNT_ASC WHERE RNUM BETWEEN 1 AND 10;
SELECT COUNT(*) FROM ADMIN_REVIEW_VIEW WHERE RE_STATUS = 1;

SELECT * FROM (SELECT ROWNUM RNUM, A.* FROM (SELECT R.*, MO_NAME, (SELECT COUNT(*) FROM REVIEW_LIKE WHERE RE_NO = LIKE_RNO) LCOUNT, BRAND_NAME, USER_NAME FROM REVIEW R LEFT JOIN MOBILE ON ( RE_MNO = MO_NO ) LEFT JOIN BRAND ON ( MO_BRANDCODE = BRAND_CODE ) LEFT JOIN USER_TABLE ON ( RE_WRITER = USER_NO ) WHERE RE_STATUS = 1 ORDER BY RE_NO DESC) A) WHERE RNUM BETWEEN 1 AND 10;

select * from mobile_view
order by "¹ß¸ÅÀÏ" DESC NULLS LAST;
