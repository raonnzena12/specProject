...
ALTER TABLE COM_COMM ADD CONSTRAINT FK_USER_TO_COM_COMM_1 FOREIGN KEY (
	CC_WIRTER
)
REFERENCES USER (
	USER_NO
...
---- 모바일 댓글 뷰 테스트
SELECT * FROM MOBILE_COMMENT_VIEW WHERE MO_NO = 42;
...
SELECT RE_NO, RE_TITLE, RE_CONTENT, RE_STAR, RE_REGDATE, RE_MODIDATE, (SELECT COUNT(*) FROM REVIEW_LIKE WHERE LIKE_UNO = 0 and re_no=like_rno GROUP BY LIKE_RNO) IM_LIKE, COUNT_L, RE_WRITER, USER_NAME, RE_MNO, RE_STATUS FROM MOBILE_REVIEW M WHERE RE_MNO = 42;
