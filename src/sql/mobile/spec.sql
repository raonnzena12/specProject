...
SELECT * FROM (SELECT ROWNUM RNUM, M.* FROM (SELECT * FROM MOBILE_VIEW WHERE STATUS = 'Y' and "제조사" in ('애플') ORDER BY 1 DESC) M ) WHERE RNUM BETWEEN 1 AND 10;
...
COMMIT;
SELECT * FROM (SELECT ROWNUM RNUM, M.* FROM (SELECT * FROM MOBILE_VIEW WHERE STATUS = 'Y' AND "제조사" IN ( '삼성전자','LG전자'  ) ORDER BY 1 DESC) M ) WHERE RNUM BETWEEN 1 AND 10;