...
public class MyReplyListServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(loginUser != null) {
	    	...
	    	// --------------------- 페이징 처리 -------------------------
	    	// 전체 게시글 수 구하기
	    	int myReplyCount = mService.getMyReplyCount(loginUser.getUserNo());
	    	...
	    	maxPage = 	(int)Math.ceil((double)myReplyCount / limit) ;
	    	...
	    	BoardPageInfo bpi = new BoardPageInfo(myReplyCount, limit, pagingBarSize, currentPage, maxPage, startPage, endPage);
	    	
	    	// --------------------------- 페이징 바 처리 끝 -----------------------------------
	    	
	    	// -------------- 게시글 목록 조회 시작 ------------------
	    	// 전체 목록을 가져오는게 아니라 해당페이지의 게시글 목록을 가져옴
	    	ArrayList<Reply> rList = mService.selectMyReply(currentPage, limit, loginUser.getUserNo()); 
	    	...
	    	if(rList != null) {
	    		page = "views/member/myReplyListView.jsp";
